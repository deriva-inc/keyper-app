...
export async function GET(req: Request) {
    ...
    const targetUrl = `${process.env.SERVER_BASE_URL}/profiles/${profileId}/entries/count`;
    logger.info(
        `[GET] ${targetUrl} | Fetching vault entries count for profile: ${profileId}`
    );
    ...
    try {
        ...
        return handleErrorOnServerSideAPICalls(
            error,
            `Failed to retrieve vault entries count for profile ${profileId}.`,
            HTTP_STATUS_CODE.INTERNAL_ERROR,
            `[GET] /api/profiles/${profileId}/entries/count`
        );
    }
}
